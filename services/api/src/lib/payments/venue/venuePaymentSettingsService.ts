...
import { createPaymentAuditEntry } from "./paymentConfigAudit.js";
import { assertMethodCanEnable } from "./paymentMethodReadiness.js";
import { METHOD_DISABLE_POLICY } from "./paymentMethodStateMachine.js";
import { secretConfiguredLabel } from "../providers/credentialVault.js";
import type { PaymentProviderConnectionRecord } from "../providers/providerConnectionTypes.js";
...
export type PaymentProviderState = {
  ...
  /** Legacy mirror of connection verification — prefer providerConnections. */
...
export type VenuePaymentSettings = {
  ...
  /** Secure provider connection records (ciphertext secrets). */
  providerConnections?: {
    stripe?: import("../providers/providerConnectionTypes.js").PaymentProviderConnectionRecord;
    swish?: import("../providers/providerConnectionTypes.js").PaymentProviderConnectionRecord;
    terminals?: import("../providers/providerConnectionTypes.js").PaymentProviderConnectionRecord;
  };
  /** Context-specific preferred methods. */
  preferencePolicies?: Partial<
    Record<
      import("./paymentPreferencePolicy.js").PaymentPreferenceContext,
...
export function mergeSettings(raw: unknown): VenuePaymentSettings {
  ...
  return {
    ...
    providerConnections: {
      ...(DEFAULT_SETTINGS.providerConnections ?? {}),
...
function appendAudit(
  ...
  const next = createPaymentAuditEntry({
...
/** Strip encrypted secrets before returning settings over ordinary APIs. */
export function toPublicVenuePaymentSettings(settings: VenuePaymentSettings): VenuePaymentSettings {
  const connections = settings.providerConnections ?? {};
  const publicConnections: VenuePaymentSettings["providerConnections"] = {};
  for (const key of Object.keys(connections) as Array<keyof NonNullable<typeof connections>>) {
    const record = connections[key] as PaymentProviderConnectionRecord | undefined;
    if (!record) continue;
    publicConnections[key] = {
      ...record,
      encryptedApiSecret: null,
      encryptedCertificate: null,
      encryptedWebhookSecret: null,
      // Keep flags for UI; never include ciphertext.
...
export function publicProviderConnectionLabels(record: PaymentProviderConnectionRecord) {
  return {
    provider: record.provider,
    connected: record.connected,
    displayName: record.displayName,
    environment: record.environment,
    merchantId: record.publicMerchantId ?? null,
    accountId: record.publicAccountId ?? null,
...
export type PaymentProviderEnvReady = ReturnType<typeof getPaymentProviderEnvReady>;
...
export async function updateVenuePaymentSettings(
  ...
  // Readiness SSOT: never enable a method the backend has not marked READY (or sandbox-verified).
  const envReadyForGuards = getPaymentProviderEnvReady();
  const previewForGuards = mergeSettings({
    ...current.settings,
    providers: {
  ...
  const enableBlocked: string[] = [];
  for (const key of Object.keys(nextMethods) as PaymentMethodKey[]) {
    const wasEnabled = Boolean(current.settings.methods[key] || current.settings.methodConfig[key]?.enabled);
    const willEnable = Boolean(nextMethods[key]);
    if (willEnable && !wasEnabled) {
      const check = assertMethodCanEnable(previewForGuards, envReadyForGuards, key);
      if (!check.ok) {
        nextMethods[key] = false;
  ...
  const enabledTransitions: Array<{ key: string; from: boolean; to: boolean }> = [];
  for (const key of Object.keys(nextMethods) as PaymentMethodKey[]) {
    const was = Boolean(current.settings.methods[key] || current.settings.methodConfig[key]?.enabled);
    const will = Boolean(nextMethods[key]);
    if (was !== will) enabledTransitions.push({ key, from: was, to: will });
  }

  let auditAction =
    audit?.action ??
    (methodKeysPatched.length === 1 ? "payment_method_updated" : "payment_settings_updated");
  if (!audit?.action && enabledTransitions.length === 1) {
    auditAction = enabledTransitions[0]!.to ? "payment.method_enabled" : "payment.method_disabled";
  } else if (
    !audit?.action &&
    patch.defaultPaymentMethodKey !== undefined &&
    patch.defaultPaymentMethodKey !== current.settings.defaultPaymentMethodKey
  ) {
    auditAction = "payment.method_default_changed";
  }
  ...
  next.auditLog = appendAudit(current.settings, {
    ...
    newValue:
      methodKeysPatched.length === 1
        ? {
            ...(next.methodConfig[methodKeysPatched[0] as PaymentMethodKey] ?? {}),
            ...(enabledTransitions[0] && !enabledTransitions[0].to
...
export async function connectPaymentProvider(
  ...
  const { paymentProviderAdapters } = await import("../providers/providerAdapter.js");
  const { emptyConnection } = await import("../providers/providerConnectionTypes.js");
  ...
  const patch: Partial<VenuePaymentSettings> = {
    providers: {
      stripe:
        provider === "stripe"
          ? {
              ...
              accountId: connection.publicAccountId,
      ...
      swish:
        provider === "swish"
          ? {
              connected: connection.connected,
              merchantId: connection.publicMerchantId,
              connectedAt: connection.connectedAt ?? undefined,
              displayName: connection.displayName,
              environment: connection.environment,
              verificationStatus: connection.verificationStatus,
    ...
    // Do NOT auto-enable methods — readiness + explicit enable required.
  };

  const result = await updateVenuePaymentSettings(prisma, restaurantId, patch, {
    ...audit,
    action: "payment.provider_connected",
...
export async function disconnectPaymentProvider(
  ...
  const adapter = paymentProviderAdapters[provider];
  const existing = current.settings.providerConnections?.[provider] ?? emptyConnection(provider);
  const disconnected = await adapter.disconnect({ restaurantId, envReady, connection: existing });

  // Safe disconnect: disable related methods for NEW attempts, preserve connection audit/history.
  const methods = { ...current.settings.methods };
  const methodConfig = { ...current.settings.methodConfig };
  const related =
    provider === "stripe"
      ? (["card", "visa", "mastercard", "amex", "applePay", "googlePay", "samsungPay", "klarnaPayNow", "klarnaPayLater", "klarnaInstallments"] as PaymentMethodKey[])
      : (["swish", "swishAtVenue"] as PaymentMethodKey[]);
  for (const key of related) {
    methods[key] = false;
    if (methodConfig[key]) methodConfig[key] = { ...methodConfig[key]!, enabled: false };
  }
  ...
  const patch: Partial<VenuePaymentSettings> = {
    providers: {
      stripe:
        provider === "stripe"
          ? { connected: false, environment: "sandbox", verificationStatus: "revoked", health: "unknown" }
          : current.settings.providers.stripe,
      swish:
        provider === "swish"
          ? { connected: false, environment: "sandbox", verificationStatus: "revoked", health: "unknown" }
    ...
    providerConnections: {
      ...(current.settings.providerConnections ?? {}),
      [provider]: disconnected.connection
    },
    methods,
    methodConfig
  };
  ...
  return updateVenuePaymentSettings(prisma, restaurantId, patch, {
    ...audit,
    action: "payment.provider_disconnected",
    path: `providerConnections.${provider}`
  });
}
