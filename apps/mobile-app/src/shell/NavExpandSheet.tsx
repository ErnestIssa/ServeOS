...
export function computeNavSheetSnapDims(screenH: number, insets: EdgeInsets) {
  ...
  const hMid = Math.max(140, Math.min(dockMaxH * 0.48, fullH * 0.42));
  return {
...
export function buildSheetPan({
  ...
  return Gesture.Pan()
    ...
    .failOffsetX([-140, 140])
    .onBegin(() => {
      startH.value = sheetHeightSV.value;
    ...
    .onEnd((e) => {
      ...
      const t = resolveSheetSnapTargetWorklet(biasedH, vy, snapMid, max);
...
export function useNavSheetPanGestures(insets: EdgeInsets, sheetHeightSV: SharedValue<number>) {
  ...
  const panVerticalOnSheetBody = React.useMemo(
    () => buildSheetPan(deps),
    [sheetHeightSV, startH, snapMidSV, snapHighSV, sheetPanDragSessionSV]
  );
