...
/** Watch workspace source directly so shared edits HMR without rebuilding all pre-bundled deps. */
function serveosWorkspaceSource(): Plugin {
  const watchDirs = [
    coreSharedSrc,
    coreAmbientSrc,
    coreLoadingSrc,
    coreThemeSrc,
    path.join(repoRoot, "apps/Agents/src")
  ];

  return {
    name: "serveos-workspace-source",
